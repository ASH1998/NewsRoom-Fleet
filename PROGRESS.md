...
| Tests | **33 passing** — policy rules, state machine, screening, runner failure/timeout/idempotency, watcher, golden pipeline, API |
...
- [ ] **Gemma PII pass has never run.** The code path is written but untested; it needs
      one live call against `gemma-4-26b-a4b-it` to confirm.
