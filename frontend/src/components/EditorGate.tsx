...
export function EditorGate({
  ...
  // Verdicts an editor has already resolved with a recorded decision. The verdict
  // records are never rewritten — the display acknowledges the decision on top.
  ...
  return (
    ...
    >
      {/* After publication the gate report still reads HUMAN_REVIEW: the editor
          resolved the blocking verdicts, but nothing rewrote them. Rows an editor
          has resolved render green — resolved, not rewritten. */}
      {published && (
      ...
      <ul className="space-y-1.5">
        {blockingVerdicts.map((verdict: Verdict) => {
          const decided = decidedIds.has(verdict.verdict_id);
          return (
            <li
            ...
            >
              <label className="flex cursor-pointer items-start gap-2">
                <input
                  type="checkbox"
                  disabled={!isEditor || published || decided}
                  checked={decided || resolved.includes(verdict.verdict_id)}
                  ...
                  <span className="text-[11px] text-stone-300">
                    ...
                    {decided ? (
                      <span className="text-emerald-300">
                        {verdict.result} · resolved by editor decision
                      </span>
                    ) : (
